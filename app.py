...
def mostrar_ultimo_envase():
    ...
    ventana_ultimo_envase = Toplevel()
    ...
    frame_contenedor = Frame(ventana_ultimo_envase, bg="#50C878")
    frame_contenedor.pack(fill=BOTH, expand=True, padx=10, pady=10)

    # Título de la ventana
    Label(frame_contenedor, text="Último Envase de Medicaciones", font=("Arial", 14, "bold"), bg="#50C878", fg="white").pack(pady=10)
...
def mostrar_estadisticas(filtro, valor_filtro):
    """
    Muestra las estadísticas de medicación en función del filtro seleccionado.
    """
    ...
    Button(frame_contenedor, text="Exportar a CSV", command=lambda: exportar_a_csv(estadisticas), bg="#007C5C", fg="white", font=("Arial", 12)).pack(pady=10)
    Button(frame_contenedor, text="Exportar a Excel", command=lambda: exportar_a_excel(estadisticas), bg="#007C5C", fg="white", font=("Arial", 12)).pack(pady=10)

    # Botón para generar gráfico
    Button(frame_contenedor, text="Generar Gráfico", command=lambda: generar_grafico(estadisticas), bg="#FF5733", fg="white", font=("Arial", 12)).pack(pady=10)
...
def generar_grafico(df):
    ...
    """
    # Agrupar por Medicación y Año para visualizar los totales de unidades pautadas por medicación
...
def pantalla_filtro_estadisticas():
    ...
    filtro_var.set("medicacion")  # Valor predeterminado
...
def proxima_dispensacion():
    ...
    ventana_dispensacion.geometry("900x600")
    ...
    def mostrar_medicaciones(event):
        ...
        for medicacion in medicaciones:
            ...
            # (unidades_por_caja / posologia) * intervalo_dias
            dias_cubiertos = max(1, unidades_por_caja // posologia)
